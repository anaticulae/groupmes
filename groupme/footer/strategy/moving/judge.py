...
class MovingFooterResultReport(groupme.footer.strategy.FooterStrategyReport):
    footer: int = None
    header: int = None
...
def analyze(results) -> MovingFooterResultReport:
    footer_count = groupme.footer.strategy.count_footer(results)
...
def footnote_number_error(numbers: list) -> bool:  # pylint:disable=R0911
    ...
    if error_rate > FOOTNOTE_NUMBER_ERROR_MAX:
        if groupme.footer.strategy.moving.utils.isendnote(numbers):
            return False
        return True
    return False
